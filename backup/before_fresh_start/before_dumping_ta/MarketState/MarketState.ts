...
export class MarketState implements IMarketState {
    ...
    private readonly maPeriods: number[] = [12, 26, 52, 78, 98, 115, 121, 159];
    ...
    private readonly dust: number[] = [0.03, 0.03, 0.03, 0.03, 0.03];
     ...
     */
     ...
     private readonly verboseForecastDetails: string[] = ['fosc']
    ...
    /*public async forecast(series: ICandlestickSeries): Promise<IForecastProviderResult> {
        ...
        // Initialize the EMAs
        const shortEMA: EMA = new EMA(5);
        let shortEMAList: number[] = [];
        const longEMA: EMA = new EMA(30);
        let longEMAList: number[] = [];

        for (let i = 0; i < data.close.length; i++) {
            shortEMA.update(data.close[i]);
            longEMA.update(data.close[i]);
            if (shortEMA.isStable && longEMA.isStable) {
                shortEMAList.push(Number(shortEMA.getResult().toFixed(2)));
                longEMAList.push(Number(longEMA.getResult().toFixed(2)));
            }
        }   
        ...
        const crossoverList: number[] = await tulind.indicators.crossany.indicator([shortEMAList, longEMAList], []);
        const crossover: number = crossoverList[0][crossoverList.length - 1];
        if (crossover) {
            const sEMA: number = Number(shortEMA.getResult().toFixed(2));
            const lEMA: number = Number(longEMA.getResult().toFixed(2));
            const price: number = data.close[data.close.length - 1];
            

            if (sEMA > lEMA) {
                console.log(`Short EMA: ${sEMA} | Long EMA: ${lEMA} | Price: ${price}`);
                tendency = 1; 
            } 
    ...
    public async forecast(series: ICandlestickSeries): Promise<IForecastProviderResult> {
        ...
        // Initialize the EMAs
        const shortEMA: number[] = await this.ema(data.close, 6);
        const longEMA: number[] = await this.ema(data.close, 31);

        // Init the tendency
        let tendency: ITendencyForecast = 0;

        const crossoverList: number[] = await tulind.indicators.crossover.indicator([shortEMA, longEMA], []);
        const crossover: number = crossoverList[0][crossoverList.length - 1];
        if (crossover) {
            const sEMA: number = shortEMA[shortEMA.length - 1];
            const lEMA: number = longEMA[longEMA.length - 1];
            const price: number = data.close[data.close.length - 1];
            

            if (sEMA > lEMA && lEMA < price) {
                console.log(`SMA 5: ${sEMA} | SMA 30: ${lEMA} | Price: ${price}`);
                tendency = 1; 
            } 
            
            else if (lEMA > sEMA && lEMA > price){
                console.log(`SMA 5: ${sEMA} | SMA 30: ${lEMA} | Price: ${price}`);
                tendency = -1;
            }
        }
    ...
    /*public async forecast(series: ICandlestickSeries): Promise<IForecastProviderResult> {
        ...
        // Initialize the MACD
        const m: IMACDResponse = await this.macd(data.close);

        let tendency: ITendencyForecast = 0;

        const crossoverList: number[] = await tulind.indicators.crossover.indicator([m.signal, m.macd], []);
        const crossover: number = crossoverList[0][crossoverList.length - 1];
        if (crossover) {
            const signal: number = m.signal[m.signal.length - 1];
            const macd: number = m.macd[m.macd.length - 1];
            const histogram: number = m.histogram[m.histogram.length - 1];
            

            if (signal > macd && histogram < signal) {
                console.log(`signal: ${signal} | macd: ${macd} | hist: ${histogram}`);
                tendency = 1; 
            } 
    ...
    private async macd(close: number[]): Promise<IMACDResponse> {
        // Caclulate the MACD
        const macd: any[] = await tulind.indicators.macd.indicator([close], [
            12, 26, 9
        ]);
    ...
    /*public async forecast(series: ICandlestickSeries): Promise<IForecastProviderResult> {
        ...
        const mas: IMovingAverages = await this.buildMovingAverages(data.open, data.high, data.low, data.close, data.volume);
    ...
    private async buildMovingAverages(open, high, low, close: number[], volume): Promise<IMovingAverages> {
        // Init value
    ...
    private forecastTendencyByAroonSummary(tendency: ITendencyForecast, summary: IAroonSummary): ITendencyForecast {
        ...
        if (tendency == 1 && summary.highPercent < 80) {
            return 0;
        }
        // Only short if it the high percent is dominating
        else if (tendency == -1 && summary.lowPercent < 80) {
            return 0;
        }
    ...
    private forecastTendencyByFOSCSummary(tendency: ITendencyForecast, summary: IFOSCSummary): ITendencyForecast {
        /*if (summary.longPercentage >= 95) {
            return 1;
        } else if (summary.shortPercentage >= 95) {
            return -1
        }
        else {
            return 0;
    ...
    private async buildRSISummary(close: number[]): Promise<IRSISummary> {
        ...
        for (let period of this.maPeriods) {
            ...
            // Check the current RSI state
            const currentRSI: number = _utils.roundNumber(rsi[rsi.length - 1], 0, true);
            ...
            else if (currentRSI <= 10) {
                oversold = true;
            }

            // Add the current value to the list
            rsiPeriods.push(currentRSI);
        }
     ...
     */
    /*private async ema(open: number[], high: number[], low: number[], close: number[], volume: number[], period?: number): Promise<number[]> {
        // Caclulate the EMA
        const ema: [number[]] = await tulind.indicators.zlema.indicator([close], [
